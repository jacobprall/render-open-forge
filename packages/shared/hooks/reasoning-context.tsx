...
export function ReasoningProvider({ children }: { children: ReactNode }) {
  ...
  const getThinkingState = useCallback((messageId: string): ThinkingState => {
    const duration = durationsRef.current.get(messageId);
    if (!duration) return { isThinking: false, thinkingDuration: null };
    if (!duration.endTime) return { isThinking: true, thinkingDuration: null };
  ...
  return (
    <ReasoningContext.Provider value={{ startReasoning, endReasoning, getThinkingState }}>
      {children}
