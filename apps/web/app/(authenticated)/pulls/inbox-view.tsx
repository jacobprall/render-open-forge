  ...
  if (items.length === 0) {
    return (
      <div className="border border-stroke-subtle bg-surface-1 px-6 py-16 text-center">
        ...
        <h3 className="text-lg font-medium text-text-secondary">All caught up</h3>
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          disabled={isPending}
          className="rounded px-2.5 py-1 text-xs font-medium text-text-tertiary transition-colors duration-(--of-duration-instant) hover:bg-surface-2 hover:text-text-primary disabled:opacity-50"
        >
          Mark all read
        </button>
      </div>
...
function InboxCard({
  ...
  return (
    <div className="flex items-start gap-4 border-b border-stroke-subtle px-5 py-4 last:border-b-0 transition-colors duration-(--of-duration-instant) hover:bg-surface-2">
      {/* Action icon */}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <Link href={`/${item.repoPath}`} className="text-xs text-text-tertiary hover:text-text-secondary transition-colors duration-(--of-duration-instant)">
            {item.repoPath}
          </Link>
          <span className="text-xs text-text-tertiary">·</span>
        ...
        <div className="mt-1">
          <Link
            href={prUrl}
            className="text-sm font-medium text-text-primary hover:text-accent-text transition-colors duration-(--of-duration-instant)"
          >
            {item.title ?? `PR #${item.prNumber}`}
          </Link>
          <span className="ml-2 text-xs text-text-tertiary">#{item.prNumber}</span>
        </div>
        <p className="mt-0.5 text-xs text-text-tertiary">{config.description}</p>

        {item.metadata?.commentBody != null && (
          <div className="mt-2 border border-stroke-default bg-surface-2 px-3 py-2 text-xs text-text-tertiary">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Link
          href={prUrl}
          className="rounded px-2 py-1 text-xs font-medium text-text-tertiary transition-colors duration-(--of-duration-instant) hover:bg-surface-3 hover:text-text-primary"
        >
          View
        </Link>
        <Link
        ...
        <button
          ...
          title="Dismiss"
          className="rounded p-1 text-text-tertiary transition-colors duration-(--of-duration-instant) hover:bg-surface-3 hover:text-text-secondary"
        >
