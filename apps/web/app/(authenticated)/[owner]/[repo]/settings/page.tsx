...
export default async function RepoSettingsPage({
  ...
  return (
    <div className="space-y-8">
      {/* General */}
      <section>
        <h2 className="mb-4 text-lg font-semibold text-text-primary">General</h2>
        <div className="border border-stroke-subtle bg-surface-1 p-6">
          <div className="space-y-4">
            <div>
              <div className="text-sm font-medium text-text-tertiary">Repository Name</div>
              <div className="mt-1 text-text-primary">{repoData.fullName}</div>
            </div>
            <div>
              <div className="text-sm font-medium text-text-tertiary">Default Branch</div>
              <div className="mt-1 inline-flex items-center gap-1.5">
                ...
                <span className="bg-surface-2 px-2 py-0.5 font-mono text-sm text-text-primary">
            ...
            {repoData.description && (
              <div>
                <div className="text-sm font-medium text-text-tertiary">Description</div>
                <div className="mt-1 text-sm text-text-secondary">{repoData.description}</div>
              </div>
            )}
            <div>
              <div className="text-sm font-medium text-text-tertiary">Visibility</div>
      ...
      {mirrorRow && (
        <section>
          <h2 className="mb-4 text-lg font-semibold text-text-primary">Mirror</h2>
          <MirrorSync
      ...
      <section>
        <h2 className="mb-4 text-lg font-semibold text-text-primary">Branch protection</h2>
        <BranchProtectionSettings owner={owner} repo={repo} defaultBranch={repoData.defaultBranch} />
      </section>

      <section>
        <h2 className="mb-4 text-lg font-semibold text-text-primary">Agent Configuration</h2>
        <PipelineEditor owner={owner} repo={repo} />
      </section>

      <section>
        <h2 className="mb-4 text-lg font-semibold text-text-primary">Secrets</h2>
      ...
      <section>
        ...
        <div className="border border-red-500/20 bg-red-500/5 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-medium text-text-primary">Delete this repository</h3>
